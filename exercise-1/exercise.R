...
ggplot(diamonds.1000, mapping = aes(x = carat, y = price)) +
  geom_point(color = "blue")

# Draw a scatter plot for `diamonds.sample` of `price` by `carat`, where each
# point has an aesthetic _shape_ based on the diamond's `cut`.
ggplot(diamonds.1000, mapping = aes(x = carat, y = price, shape = cut)) +
  geom_point(color = "blue")
...
ggplot(diamonds.1000, mapping = aes(x = carat, y = price, color = carat)) +
  geom_point() +
